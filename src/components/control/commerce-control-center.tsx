...
export function CommerceControlCenter({ ai, billing, paymentAttempts, policy, plans, tenants }: { ai: AiControl; billing: BillingControl; paymentAttempts: PaymentAttempt[]; policy: Policy; plans: Plan[]; tenants: TenantRow[] }) {
        ...
        <section className="rounded-2xl border bg-card p-5 shadow-sm">
            ...
            <div className="mt-5 overflow-x-auto">
                <table className="w-full min-w-[900px] text-left text-sm">
                    <thead className="border-b text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-3 py-2">Negocio</th><th className="px-3 py-2">Plan</th><th className="px-3 py-2">Importe</th><th className="px-3 py-2">Estado</th><th className="px-3 py-2">Pago</th><th className="px-3 py-2">Fecha</th></tr></thead>
                    <tbody>{paymentAttempts.length ? paymentAttempts.map((attempt) => <tr key={attempt.id} className="border-b last:border-0"><td className="px-3 py-3"><p className="font-medium">{attempt.tenantName}</p><p className="text-xs text-muted-foreground">/{attempt.tenantSlug}</p></td><td className="px-3 py-3">{attempt.planName}</td><td className="px-3 py-3">{new Intl.NumberFormat("es-MX", { style: "currency", currency: attempt.currency }).format(attempt.amountCents / 100)}</td><td className="px-3 py-3"><p className="font-medium">{attempt.status}</p><p className="text-xs text-muted-foreground">{attempt.lastProviderStatus || "Sin notificación"}</p>{attempt.lastError ? <p className="max-w-64 truncate text-xs text-destructive" title={attempt.lastError}>{attempt.lastError}</p> : null}</td><td className="px-3 py-3 font-mono text-xs">{attempt.providerPaymentId || "—"}</td><td className="px-3 py-3">{new Intl.DateTimeFormat("es-MX", { dateStyle: "medium", timeStyle: "short" }).format(new Date(attempt.paidAt || attempt.createdAt))}</td></tr>) : <tr><td className="px-3 py-6 text-center text-muted-foreground" colSpan={6}>Todavía no hay intentos de cobro con Mercado Pago.</td></tr>}</tbody>
                </table>
            </div>
        </section>
