...
export function EmailVerification() {
    ...
    useEffect(() => {
        ...
        void fetch("/api/public/signup-intents/verify", {
            ...
            const data = await response.json() as { verified?: boolean; onboardingPath?: string; trialEligible?: boolean };
            if (!active) return;
    ...
    return <section className="w-full rounded-2xl border bg-card p-6 shadow-sm sm:p-8">
        ...
        {state === "verified" ? <><CheckCircle2 className="size-8 text-emerald-600" /><h1 className="mt-5 text-2xl font-semibold">Correo confirmado</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">{trialEligible ? "Tu espacio se está preparando. Inicia sesión para continuar con la configuración inicial." : "Este correo ya utilizó su período de prueba. Prepararemos tu espacio y podrás continuar eligiendo un plan."}</p><Link href={`/login?redirectTo=${encodeURIComponent(onboardingPath || "/tenants")}`} className="mt-6 inline-flex h-11 items-center rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground">Iniciar sesión y continuar</Link></> : null}
