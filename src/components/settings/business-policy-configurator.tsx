...
import { ArrowLeft, ArrowRight, CalendarClock, Scissors, ShieldCheck, Store, UserRound, UserRoundCheck } from "lucide-react";
...
import { HUMAN_ESCALATION_TRIGGERS, type BusinessPolicies, type HumanEscalationTrigger } from "@/lib/ai/business-policies";
...
const STEPS = [
    ...
    { title: "Políticas", icon: ShieldCheck },
...
const TRIGGER_LABELS: Record<HumanEscalationTrigger, string> = {
    ...
    payment_issue: "Reporta una situación que requiere revisión del equipo",
...
export function BusinessPolicyConfigurator({ value, onChange, agentName, onAgentNameChange, escalationPhone, onEscalationPhoneChange, canManageAi = false }: ConfiguratorProps) {
    ...
    return <div className="mt-5 rounded-2xl border bg-background p-4 sm:p-6">
        ...
        <div className="min-h-[390px] border-b px-1 py-6 sm:px-2">
            ...
            {step === 1 ? <BusinessStep value={value} update={update} /> : null}
...
function BusinessStep({ value, update }: StepProps) {
    return <div className="space-y-6"><div><Label>Enlace público de Google Maps (opcional)</Label><Input className="mt-2 placeholder:text-muted-foreground/35" type="url" maxLength={500} value={value.publicInfo.mapsUrl} onChange={(event) => update({ publicInfo: { mapsUrl: event.target.value } })} placeholder="https://maps.app.goo.gl/..." /></div><p className="rounded-xl border bg-muted/20 p-4 text-sm text-muted-foreground">La ubicación se comparte cuando ayuda a la persona a llegar al negocio.</p></div>;
}
