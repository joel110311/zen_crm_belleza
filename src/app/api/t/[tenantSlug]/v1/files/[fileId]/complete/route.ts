...
import { confirmPrivateFileUpload } from "@/lib/tenant-private-files";
...
export async function POST(request: Request, { params }: { params: Promise<{ tenantSlug: string; fileId: string }> }) {
    ...
    return withTenantApi(request, tenantSlug, { operation: "write", permission: "files.write" }, async (tenant) => {
        const file = await confirmPrivateFileUpload({ db: tenant.db, fileId });
        return tenantData({ file: { id: file.id, status: file.status, antivirusStatus: file.antivirusStatus, confirmedAt: file.confirmedAt?.toISOString() || null } }, tenant.requestId);
    });
}
