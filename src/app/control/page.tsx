...
export default async function ControlCenterPage() {
    ...
    const [policy, plans, activeTrials, endingTrials, activeSubscriptions, failedPayments, failedCheckoutAttempts, paymentAttempts, tenants, verifiedSignups, convertedTrials] = await Promise.all([
        ...
        db.billingCheckoutAttempt.findMany({
            ...
            take: 50,
            include: { tenant: { select: { displayName: true, slug: true } }, plan: { select: { name: true } } },
        }),
    ...
    return <main className="min-h-dvh bg-muted/20 px-4 py-8 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><header><p className="text-sm font-semibold text-primary">Administración de plataforma</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Centro de mando comercial</h1><p className="mt-2 text-sm text-muted-foreground">Pruebas, planes, cobros, inteligencia y conversión sin entrar a los datos operativos de cada negocio.</p></header><section className="my-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Metric label="Pruebas activas" value={activeTrials} /><Metric label="Vencen en 48 h" value={endingTrials} /><Metric label="Suscripciones activas" value={activeSubscriptions.length} /><Metric label="Pagos con atención" value={failedPayments + failedCheckoutAttempts} /><Metric label="MRR estimado" value={new Intl.NumberFormat("es-MX", { style: "currency", currency: "MXN", maximumFractionDigits: 0 }).format(mrr / 100)} detail={`${conversion}% conversión`} /></section><CommerceControlCenter ai={{ chatModel: ai.chatModel, geminiKeyConfigured: ai.geminiKeyConfigured, openaiKeyConfigured: ai.openaiKeyConfigured, environmentFallbackEnabled: ai.environmentFallbackEnabled }} billing={{ provider: billingProvider, mercadoPagoEnabled: process.env.MERCADO_PAGO_ENABLED === "true", applicationIdConfigured: Boolean(process.env.MERCADO_PAGO_APPLICATION_ID?.trim()), accessTokenConfigured: Boolean(process.env.MERCADO_PAGO_ACCESS_TOKEN?.trim()), webhookSecretConfigured: Boolean(process.env.MERCADO_PAGO_WEBHOOK_SECRET?.trim()) }} paymentAttempts={paymentAttempts.map((attempt) => ({ id: attempt.id, tenantName: attempt.tenant.displayName, tenantSlug: attempt.tenant.slug, planName: attempt.plan.name, amountCents: attempt.amountCents, currency: attempt.currency, status: attempt.status, lastProviderStatus: attempt.lastProviderStatus, providerPaymentId: attempt.providerPaymentId, createdAt: attempt.createdAt.toISOString(), paidAt: attempt.paidAt?.toISOString() || null, lastError: attempt.lastError }))} policy={{ trialDays: policy.trialDays, warningHours: policy.warningHours, finalWarningHours: policy.finalWarningHours, graceDays: policy.graceDays, version: policy.version }} plans={plans.map((plan) => ({ id: plan.id, slug: plan.slug, name: plan.name, monthlyAmountCents: plan.monthlyAmountCents, stripePriceId: plan.prices[0]?.externalPriceId || "" }))} tenants={tenants.map((tenant) => ({ id: tenant.id, displayName: tenant.displayName, slug: tenant.slug, accessMode: tenant.accessMode, billingStatus: tenant.billingStatus, trialEndsAt: tenant.trial?.endsAt.toISOString() || null, trialStatus: tenant.trial?.status || null, selectionStatus: tenant.billingSelection?.status || null, lastError: tenant.billingSelection?.lastError || null, qrProxyEnabled: tenant.qrChannelConfiguration?.proxyEnabled || false, qrProxyConfigured: Boolean(tenant.qrChannelConfiguration?.proxyUrlCiphertext) }))} /></div></main>;
}
